    ...
    const TESTIMONIALS = [
        {
            name: 'Aditi Rao',
            role: 'Student, Delhi',
            quote: 'Nirbhaya has given me the confidence to attend late-night study sessions. The live map is incredibly accurate.',
            rating: 5,
    ...
    return (
        <section className="py-20 lg:py-32 bg-[#F5F1E8]">
            <div className="max-w-7xl mx-auto px-8 text-center mb-20 space-y-4">
                <h2 className="text-4xl font-bold font-serif text-[#2c2a1e]">What Women Say About Us</h2>
                <div className="flex justify-center gap-1">
                    {[1, 2, 3, 4, 5].map((i) => (
                        <Star key={i} className="w-5 h-5 fill-[#a8b86e] text-[#a8b86e]" />
                    ))}
                </div>
            </div>
            ...
            <div className="max-w-7xl mx-auto px-8 grid md:grid-cols-3 gap-8">
                {TESTIMONIALS.map((t, i) => (
                    <div key={i} className="bg-white p-10 rounded-[32px] shadow-sm relative border border-gray-100 flex flex-col items-center text-center">
                        ...
                        <p className="text-[#2c2a1e] font-body italic mb-6 leading-relaxed">
                            "{t.quote}"
                        </p>
                        <div>
                            <p className="font-serif font-bold text-[#7a8a42]">{t.name}</p>
                            <p className="text-[10px] text-[#6b6550] font-bold uppercase tracking-widest mt-1">{t.role}</p>
                        </div>
                    </div>
                ))}
            </div>
