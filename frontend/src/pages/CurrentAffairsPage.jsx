...
import { Newspaper, Bell, MapPin, Share2, Calendar, ShieldAlert, TrendingUp, ExternalLink, ChevronLeft, ChevronRight, X, Radio } from 'lucide-react';
...
export default function CurrentAffairsPage() {
    ...
    const NEWS = [
        {
            ...
            image: 'https://images.unsplash.com/photo-1589578228447-e1a4e481c6c8?auto=format&fit=crop&q=80&w=600',
    ...
    const BREAKING_TICKER = [
        ...
        'Night cabs for women get Rs.50 subsidy under "Safe Ride" scheme in Ahmedabad'
    ];
    ...
    return (
        <div className="max-w-7xl mx-auto px-8 py-12 relative">
            ...
            <div className="mb-10 rounded-2xl overflow-hidden shadow-xl border border-[#8A2B57]/10" style={{ background: 'linear-gradient(135deg, #1a0a12 0%, #2d1020 50%, #1a0a12 100%)' }}>
                <div className="flex items-center">
                    {/* LIVE badge */}
                    <div className="px-5 py-4 flex items-center gap-2.5 shrink-0" style={{ background: 'linear-gradient(135deg, #DC2626, #991B1B)' }}>
                        <span className="relative flex h-2.5 w-2.5">
                            <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-white opacity-75" />
                            <span className="relative inline-flex rounded-full h-2.5 w-2.5 bg-white" />
                        </span>
                        <span className="text-white text-[10px] font-black uppercase tracking-[0.25em]">Live</span>
                    </div>

                    {/* Ticker content */}
                    <div className="flex-1 px-6 py-4 flex items-center justify-between overflow-hidden min-h-[52px]">
                        <div className="flex-1 overflow-hidden relative">
                            <p
                                key={tickerIndex}
                                className={`text-white/90 font-medium text-sm leading-relaxed transition-all duration-500 ease-out ${
                                    tickerFade
                        ...
                        <div className="flex items-center gap-2 ml-6 shrink-0">
                            <button
                                onClick={() => { setTickerFade(false); setTimeout(() => { setTickerIndex(prev => (prev - 1 + BREAKING_TICKER.length) % BREAKING_TICKER.length); setTickerFade(true); }, 250); }}
                                className="p-1.5 rounded-lg text-white/30 hover:text-white hover:bg-white/10 transition-all"
                            >
                                <ChevronLeft className="w-4 h-4" />
                            </button>
                            <span className="text-white/25 text-[10px] font-mono tabular-nums min-w-[28px] text-center">{tickerIndex + 1}/{BREAKING_TICKER.length}</span>
                            <button
                                onClick={() => { setTickerFade(false); setTimeout(() => { setTickerIndex(prev => (prev + 1) % BREAKING_TICKER.length); setTickerFade(true); }, 250); }}
                                className="p-1.5 rounded-lg text-white/30 hover:text-white hover:bg-white/10 transition-all"
                            >
                ...
                <div className="h-[3px] bg-white/5">
                    <div
                        className="h-full transition-all ease-linear rounded-r-full"
                        style={{
                            width: `${((tickerIndex + 1) / BREAKING_TICKER.length) * 100}%`,
                            transitionDuration: '400ms',
                            background: 'linear-gradient(90deg, #8A2B57, #DC2626)'
                        }}
                    />
                </div>
            </div>
            ...
            <header className="mb-12">
                <div className="flex items-center gap-3 mb-4 text-[#a8b86e]">
                    <TrendingUp className="w-5 h-5" />
                    <span className="text-sm font-bold uppercase tracking-[0.2em]">Latest Updates</span>
                </div>
                <h1 className="text-4xl font-bold font-serif text-[#7a8a42] leading-tight tracking-tight">Safety News & Alerts</h1>
            </header>
                ...
                <div className="mb-12 p-6 bg-red-50 border-l-4 border-[#DC2626] rounded-r-2xl flex items-start justify-between">
                    <div className="flex items-start gap-4">
                        ...
                        <div>
                            ...
                            <p className="text-sm text-red-800 font-body">Avoid SG Highway near Sola Bridge between 11 PM–5 AM due to ongoing road construction. Use Nirbhaya Verified alternate routes via Science City Road.</p>
                        </div>
                    </div>
                    <button
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
                    ...
                    {NEWS.map((item) => (
                        <article key={item.id} className="flex flex-col md:flex-row gap-8 group">
                            <div className="md:w-1/3 aspect-video md:aspect-square overflow-hidden rounded-2xl relative">
                                ...
                                />
                                <span className={`absolute top-4 left-4 px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-widest text-white shadow-lg ${
                                    item.category === 'Ahmedabad' ? 'bg-[#a8b86e]' :
                                    item.category === 'Gujarat' ? 'bg-[#7a8a42]' :
                            ...
                            <div className="md:w-2/3 flex flex-col justify-center">
                                ...
                                <h3 className="text-2xl font-bold font-serif text-[#2c2a1e] leading-snug mb-4 group-hover:text-[#7a8a42] transition-colors">
                                    {item.title}
                                </h3>
                                <p className="text-[#6b6550] font-body text-sm leading-relaxed mb-6">
                                    {item.excerpt}
                                </p>
                                <div className="flex items-center gap-6">
                                    <button 
                                        onClick={() => setExpandedArticle(expandedArticle === item.id ? null : item.id)}
                                        className="text-[#7a8a42] font-bold text-sm hover:underline decoration-2 underline-offset-4 flex items-center gap-1.5"
                                    >
                                    ...
                                    <button 
                                        ...
                                        className="text-gray-400 hover:text-[#7a8a42] transition-colors"
                                    >
                                ...
                                {expandedArticle === item.id && (
                                    <div className="mt-6 p-6 bg-[#f5f0e0] rounded-2xl border border-[#7a8a42]/10 animate-in slide-in-from-top-2">
                                        <div className="prose prose-sm max-w-none">
                                            {item.fullStory.split('\n\n').map((para, idx) => (
                                                <p key={idx} className="text-[#2c2a1e] font-body text-sm leading-relaxed mb-4 last:mb-0">
                                        ...
                                        {item.sourceUrl && (
                                            <a 
                                                ...
                                                className="inline-flex items-center gap-2 mt-4 text-[#7a8a42] font-bold text-xs uppercase tracking-widest hover:underline"
                                            >
                                                <ExternalLink className="w-3.5 h-3.5" />
                                                Visit Source
                                            </a>
                ...
                <div className="lg:col-span-1 space-y-10">
                    <section className="p-8 bg-white border border-gray-200 rounded-3xl shadow-sm">
                        <h4 className="text-xl font-bold font-serif text-[#7a8a42] mb-6 flex items-center gap-3">
                        ...
                        <div className="space-y-6 text-sm font-body">
                            {TRENDING_ALERTS.map((alert, i) => (
                                <div key={i} className="flex gap-4 group cursor-pointer hover:bg-gray-50 p-2 rounded-xl transition-colors">
                                    <span className="text-[#7a8a42] font-bold text-lg leading-none pt-1">0{i + 1}.</span>
                                    <p className="text-[#2c2a1e] leading-relaxed group-hover:text-[#7a8a42] transition-colors font-medium">
                                        {alert}
                                    </p>
                                </div>
                            ))}
                        </div>
                        <button className="w-full mt-8 py-3 bg-[#F5F1E8] text-[#7a8a42] font-bold rounded-xl hover:bg-[#7a8a42] hover:text-white transition-all text-sm uppercase tracking-widest">
                            View All Alerts
                        </button>
                    </section>
                    ...
                    <section className="p-8 bg-white border border-gray-200 rounded-3xl shadow-sm">
                        ...
                        <div className="space-y-4">
                            {[
                                { name: 'Women Helpline', number: '181', color: 'bg-[#a8b86e]' },
                                { name: 'Police Emergency', number: '112', color: 'bg-[#DC2626]' },
                                { name: 'Ahmedabad Women Cell', number: '079-25507225', color: 'bg-[#7a8a42]' },
                                ...
                                <a key={i} href={`tel:${item.number}`} className="flex items-center justify-between p-3 rounded-xl hover:bg-gray-50 transition-colors group">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <span className="text-sm font-bold text-[#2c2a1e] group-hover:text-[#7a8a42] transition-colors">{item.name}</span>
                                    </div>
                                    <span className="text-sm font-mono font-bold text-[#7a8a42]">{item.number}</span>
                                </a>
                            ))}
                        </div>
                    </section>

                    <section className="p-8 bg-[#2c2a1e] text-white rounded-3xl relative overflow-hidden">
                        ...
                        <div className="absolute -bottom-10 -right-10 w-40 h-40 bg-[#7a8a42]/20 blur-3xl" />
                    </section>
                </div>
            </div>
        </div>
